...
def gaussian_blur(img, max_filiter_size=3, sigma=0.5):
	if not _is_numpy_image(img):
		raise TypeError('img should be numpy ndarray. Got {}'.format(type(img)))
	...
	if max_filiter_size >= 3 :
		...
		if filter_size % 2 == 0 :
			filter_size += 1
		img = cv2.GaussianBlur(img, (filter_size, filter_size), sigma)
	return img

def gaussian_noise(img, mean=0, var=1):
	if not _is_numpy_image(img):
		raise TypeError('img should be numpy ndarray. Got {}'.format(type(img)))

	gaus_noise = np.random.normal(mean, var, img.shape)
	img = img + gaus_noise
	return img

# RGB to Bayer Pattern 
def stack_bayer(image_rgb):
    image_rgb = np.squeeze(image_rgb)
    h, w, _ = image_rgb.shape
    bayer = np.zeros([h, w], dtype=np.uint8)
    bayer[0:h:2,0:w:2] = image_rgb[0:h:2,0:w:2,0]#R
    bayer[1:h:2,0:w:2] = image_rgb[1:h:2,0:w:2,1]#G1
    bayer[0:h:2,1:w:2] = image_rgb[0:h:2,1:w:2,1]#G2
    bayer[1:h:2,1:w:2] = image_rgb[1:h:2,1:w:2,2]#B
    return bayer