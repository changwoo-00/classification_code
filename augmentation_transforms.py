...
class RandomRotation(object):
    ...
    def __init__(self, degrees, degree_fix=False, resample=False, expand=False, center=None, p=0.2):
    ...
    def __call__(self, img):
        ...
        angle = self.get_params(self.degrees, self.fix)
...
class RandomGaussianBlur(object):
    def __init__(self, max_filter_size=3, sigma_min=0.5, sigma_max=1.5, p=0.2):
        self.p                  = p
    ...
    def __call__(self, img):
        if random.random() < self.p:
            ...
            return F.gaussian_blur(img, filter_size, sigma)
        return img

class RandomGaussianNoise(object):
    def __init__(self, mean=0, max_var=10, p=0.2):
        self.p          = p
        self.mean       = mean
    ...
    def __call__(self, img):
        if random.random() < self.p:
            var = random.randint(0, self.max_var)
            return F.gaussian_noise(img, self.mean, var)
        return img

class RandomMakeBayerImage(object):
    def __init__(self,p=0.2):
        self.p      = p
    
    def __call__(self, img):
        img_rgb = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
        img_bayer = F.stack_bayer(img_rgb.copy())
        img_bayer = cv2.cvtColor(img_bayer, cv2.COLOR_BAYER_BG2BGR)
        return img_bayer